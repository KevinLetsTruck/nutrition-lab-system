  ...
  try {
    ...
    for (let i = 0; i < questions.length; i++) {
      for (let j = i + 1; j < questions.length; j++) {
        ...
        if (commonWords.length >= 3 && commonWords.length >= Math.min(words1.length, words2.length) * 0.5) {
          redundancies.push({
    ...
    questions.forEach(q => {
      ...
      if (text.includes('digest') || text.includes('stomach') || text.includes('bowel') || text.includes('gas') || text.includes('bloat')) {
        topics.digestive.push(q.id);
      }
      if (text.includes('energy') || text.includes('fatigue') || text.includes('tired')) {
        topics.energy.push(q.id);
      }
      if (text.includes('pain') || text.includes('ache') || text.includes('hurt')) {
        topics.pain.push(q.id);
      }
      if (text.includes('sleep') || text.includes('insomnia') || text.includes('wake')) {
        topics.sleep.push(q.id);
      }
      if (text.includes('mood') || text.includes('depress') || text.includes('anxiety') || text.includes('stress')) {
        topics.mood.push(q.id);
      }
      if (text.includes('skin') || text.includes('rash') || text.includes('acne') || text.includes('eczema')) {
        topics.skin.push(q.id);
      }
      if (text.includes('hormone') || text.includes('menstrual') || text.includes('thyroid')) {
        topics.hormonal.push(q.id);
      }
      if (text.includes('immune') || text.includes('infection') || text.includes('cold') || text.includes('flu')) {
        topics.immune.push(q.id);
      }
      if (text.includes('heart') || text.includes('blood pressure') || text.includes('chest')) {
        topics.cardiovascular.push(q.id);
      }
      if (text.includes('breath') || text.includes('lung') || text.includes('asthma')) {
        topics.respiratory.push(q.id);
      }
      if (text.includes('headache') || text.includes('dizzy') || text.includes('memory') || text.includes('focus')) {
        topics.neurological.push(q.id);
      }
      if (text.includes('joint') || text.includes('muscle') || text.includes('back')) {
        topics.musculoskeletal.push(q.id);
      }
      if (text.includes('urin') || text.includes('bladder') || text.includes('kidney')) {
        topics.urinary.push(q.id);
      }
      if (text.includes('weight') || text.includes('gain') || text.includes('lose')) {
        topics.weight.push(q.id);
      }
      if (text.includes('eat') || text.includes('food') || text.includes('diet') || text.includes('meal')) {
