// Use require to bypass TypeScript module resolution issues in scripts
const { getNextQuestionWithAI } = require("../lib/ai/assessment-ai");
...
async function testClaudeAssessment() {
  ...
  try {
    ...
    console.log(
      `- High Severity Symptoms: ${mockContext.assessmentContext.highSeveritySymptoms.length}`
    );
    ...
    console.log(
      `- Next Question: ${
        aiDecision.nextQuestion?.text || "None (assessment complete)"
      }`
    );
    ...
    console.log("\n🎯 Expected Behavior:");
    console.log(
      "- Should skip basic energy questions due to high severity (8/10)"
    );
