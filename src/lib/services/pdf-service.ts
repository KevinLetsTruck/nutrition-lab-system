...
async function generatePDFFromHTML(
  ...
  try {
    ...
    console.log(`✅ PDF generated in ${Date.now() - startTime}ms (${formatFileSize(pdfBuffer.length)})`);
 ...
 */
export async function generateProtocolPDF(request: PDFGenerationRequest): Promise<PDFGenerationResult> {
  const startTime = Date.now();
  ...
  try {
    ...
    console.log(`🔄 Starting PDF generation for protocol: ${request.protocol.protocolName}`);
    
    // Generate HTML template
    const htmlContent = generateProtocolPDFTemplate(request.protocol, templateOptions);
    ...
    // Generate filename
    const filename = request.filename || generatePDFFilename(
      request.protocol.id, 
    ...
    console.log(`🎉 PDF generation completed successfully in ${generationTime}ms`);
