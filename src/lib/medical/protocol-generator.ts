...
export class FNTPProtocolGenerator {
  ...
  private detectPatternsFromLabValues(labValues: any[]): any[] {
    ...
    const glucose = labValues.find(lab => lab.testName.toLowerCase().includes('glucose'))
    const triglycerides = labValues.find(lab => lab.testName.toLowerCase().includes('triglycerides'))
    const hdl = labValues.find(lab => lab.testName.toLowerCase().includes('hdl'))
    ...
    const tsh = labValues.find(lab => lab.testName.toLowerCase().includes('tsh'))
    const freeT4 = labValues.find(lab => lab.testName.toLowerCase().includes('t4'))
    ...
    const crp = labValues.find(lab => lab.testName.toLowerCase().includes('crp'))
    const esr = labValues.find(lab => lab.testName.toLowerCase().includes('esr'))
  ...
  private async generateSupplementProtocols(analysis: any): Promise<ClientProtocol['supplements']> {
    ...
    for (const pattern of patterns) {
      ...
      if (pattern.name === 'Chronic Inflammation') {
        ...
        phase2.push({
          ...
          priority: 2,
          roadInstructions: 'Keep refrigerated when possible, room temp okay for travel',
          source: 'biotics'
        })
      }
    }
  ...
  private async generateMealPlan(analysis: any): Promise<MealPlan> {
    const patterns = analysis.patterns || []
    const hasInsulinResistance = patterns.some((p: any) => p.name === 'Insulin Resistance')
    const hasInflammation = patterns.some((p: any) => p.name === 'Chronic Inflammation')
    ...
    return {
      phase: hasInsulinResistance ? 'Blood Sugar Stabilization' : 'Anti-Inflammatory Healing',
  ...
  private async generateLifestyleProtocols(analysis: any): Promise<LifestyleProtocol[]> {
    return [
      {
        category: 'movement',
        recommendations: [
          {
            ...
            truckerModification: 'Use rest stops, truck stops, or safe roadside areas',
            difficulty: 'easy'
          },
          {
            instruction: 'Perform cab exercises during mandatory breaks',
            frequency: 'During 30-minute breaks',
            truckerModification: 'Seated spinal twists, neck rolls, ankle pumps, shoulder shrugs',
            difficulty: 'easy'
          },
          {
            instruction: 'Resistance band workout',
            frequency: '3x per week',
            truckerModification: 'Keep bands in cab, use outside truck or in sleeper',
            difficulty: 'moderate'
          }
        ]
      },
      {
        ...
        recommendations: [
          ...
          },
          {
            ...
            truckerModification: 'Blackout curtains, eye mask, cover all LED lights',
            difficulty: 'easy'
          },
          {
            instruction: 'No screens 1 hour before sleep',
            frequency: 'Daily',
            truckerModification: 'Use blue light blocking glasses if must use devices',
            difficulty: 'moderate'
          }
        ]
      },
        ...
        recommendations: [
          {
            ...
            frequency: '3x daily',
            truckerModification: 'Practice during traffic, loading/unloading waits',
            difficulty: 'easy'
          },
          {
            instruction: 'Call family/friends regularly',
            frequency: 'Daily',
            truckerModification: 'Use hands-free during breaks, not while driving',
            difficulty: 'easy'
          },
          {
            instruction: 'Meditation or prayer',
            frequency: 'Daily',
            truckerModification: '10 minutes in sleeper before sleep or upon waking',
            difficulty: 'moderate'
          }
        ]
      },
      {
        ...
        recommendations: [
          ...
          },
          {
            ...
            frequency: 'Always',
            truckerModification: 'Windows up during fueling, avoid idling unnecessarily',
            difficulty: 'easy'
          },
          {
            instruction: 'Grounding/earthing practice',
            frequency: 'Daily',
            truckerModification: 'Walk barefoot on grass during breaks when possible',
            difficulty: 'easy'
          }
        ]
      }
  ...
  private async generateMonitoringSchedule(analysis: any): Promise<MonitoringSchedule> {
    const patterns = analysis.patterns || []
    const labsToReorder = ['Comprehensive Metabolic Panel', 'Lipid Panel']
    const symptomsToTrack = ['Energy levels (1-10 daily)', 'Sleep quality (1-10 daily)']
    ...
    patterns.forEach((pattern: any) => {
      ...
      if (pattern.name === 'Hypothyroid Pattern') {
        labsToReorder.push('TSH', 'Free T4', 'Free T3', 'Reverse T3', 'TPO Antibodies')
  ...
  private generateExecutiveSummary(analysis: any): ClientProtocol['executiveSummary'] {
    const grade = analysis.healthGrade || 'C'
    const patterns = analysis.patterns || []
    
    const keyFindings = [
      `Overall health grade: ${grade} (${analysis.overallHealthScore || 75}/100)`,
      ...patterns.map((p: any) => `${p.name} detected with ${(p.confidence * 100).toFixed(0)}% confidence`)
    ]

    const topPriorities = patterns.length > 0 
    ...
    return {
      ...
      timeframe: '6-8 weeks for initial improvements, 3-6 months for full optimization'
    }
  }

  private async generateEducation(analysis: any): Promise<ClientProtocol['education']> {
    return {
      resources: [
      ...
      explanations: [
        {
          ...
          content: 'Functional medicine uses tighter ranges that indicate optimal health rather than just absence of disease. This allows us to catch dysfunction early and optimize your health proactively.'
        },
        {
          topic: 'Truck Driver Health Challenges',
          content: 'Professional drivers face unique health challenges: sedentary work, irregular schedules, limited food access, and environmental exposures. This protocol addresses these specific challenges.'
        },
        {
          topic: 'Why These Supplements',
          content: 'Each supplement is chosen based on your specific lab findings and health patterns. We prioritize road-compatible options that fit your driving lifestyle.'
        }
      ]
    }
  }

  private async generateFollowUpSchedule(analysis: any): Promise<ClientProtocol['followUp']> {
    return {
      checkIn1Week: [
  ...
  private async saveProtocol(documentId: string, protocol: ClientProtocol): Promise<void> {
    try {
