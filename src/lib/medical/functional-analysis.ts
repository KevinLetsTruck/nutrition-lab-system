import { FUNCTIONAL_MEDICINE_RANGES, DOT_CRITICAL_VALUES } from './functional-ranges'
...
export class FunctionalMedicineAnalyzer {
  ...
  async analyzeDocument(documentId: string): Promise<FunctionalAnalysisResult> {
    console.log(`🔬 Starting functional medicine analysis for document: ${documentId}`)
    ...
    try {
      ...
      const criticalFindings = assessments.filter(a => a.status === 'critical')
      ...
      const recommendations = this.generateRecommendations(assessments, patterns)
      
      // Create next steps
      const nextSteps = this.createNextSteps(assessments, patterns, criticalFindings)
      ...
      console.log(`✅ Functional analysis complete: Grade ${overallHealth.grade}, ${patterns.length} patterns detected`)
      return result
  ...
  private assessLabValue(labValue: unknown): LabAssessment {
    ...
    const testNameKey = lab.testName.toLowerCase().replace(/\s+/g, '').replace(/[^a-z0-9]/g, '')
    
    // Try to find ranges by standard name first, then by processed test name
    let ranges = standardName ? FUNCTIONAL_MEDICINE_RANGES[standardName] : undefined
    if (!ranges) {
    ...
    if (!ranges) {
      ...
      return {
        ...
        unit: lab.unit || '',
        status: 'conventional',
        functionalStatus: lab.flag === 'normal' ? 'optimal' : 
                         lab.flag === 'low' ? 'low' : 'high',
        deviation: 0,
    ...
    if (ranges.critical && 
      ...
      deviation = lab.value <= ranges.critical.low ? 
  ...
  private detectHealthPatterns(assessments: LabAssessment[]): HealthPattern[] {
    ...
    if (irAssessments.length >= 2) {
      const elevatedGlucose = irAssessments.find(a => 
        a.testName.toLowerCase().includes('glucose') && a.functionalStatus === 'high'
      )
      const elevatedTriglycerides = irAssessments.find(a => 
        a.testName.toLowerCase().includes('triglycerides') && a.functionalStatus === 'high'
      )
      const lowHDL = irAssessments.find(a => 
        a.testName.toLowerCase().includes('hdl') && a.functionalStatus === 'low'
      )

      if ((elevatedGlucose && elevatedTriglycerides) || 
        ...
        patterns.push({
          ...
          description: 'Pattern suggesting insulin resistance and metabolic dysfunction',
          rootCauses: [
          ...
          truckDriverImpact: 'Increased risk of Type 2 diabetes, cardiovascular disease, and potential DOT medical disqualification',
          interventions: [
            {
              priority: 1,
              category: 'nutrition',
            ...
            },
            {
              ...
              recommendation: 'Chromium 400mcg, Alpha Lipoic Acid 300mg, Berberine 500mg',
              roadCompatible: true
            },
            {
              priority: 3,
    ...
    const thyroidAssessments = assessments.filter(a => 
      thyroidMarkers.some(marker => 
        a.testName.toLowerCase().replace(/\s/g, '').includes(marker.replace(/\s/g, ''))
      )
    )

    if (thyroidAssessments.length >= 1) {
      const elevatedTSH = thyroidAssessments.find(a => 
        a.testName.toLowerCase().includes('tsh') && a.functionalStatus === 'high'
      )
      const lowT4 = thyroidAssessments.find(a => 
        a.testName.toLowerCase().includes('t4') && a.functionalStatus === 'low'
      )
      const lowT3 = thyroidAssessments.find(a => 
        a.testName.toLowerCase().includes('t3') && a.functionalStatus === 'low'
      )
      ...
      if (elevatedTSH || lowT4 || lowT3) {
        patterns.push({
          ...
          truckDriverImpact: 'Fatigue, brain fog, and depression affecting driving safety and job performance',
          interventions: [
            {
              priority: 1,
              category: 'medical',
              recommendation: 'Comprehensive thyroid panel including antibodies',
              roadCompatible: false
            },
            {
              priority: 2,
              category: 'supplements',
              recommendation: 'Iodine/Kelp complex, Selenium 200mcg, Tyrosine 500mg',
              roadCompatible: true
            },
            {
              priority: 3,
              category: 'nutrition',
              recommendation: 'Avoid soy, increase sea vegetables and Brazil nuts',
              roadCompatible: true
            }
          ]
        })
    ...
    if (inflammationAssessments.some(a => a.functionalStatus === 'high')) {
      patterns.push({
        ...
        description: 'Elevated inflammatory markers indicating systemic inflammation',
        rootCauses: [
        ...
        truckDriverImpact: 'Accelerated aging, increased disease risk, and reduced recovery from stress',
        interventions: [
          {
            priority: 1,
            category: 'nutrition',
            recommendation: 'Anti-inflammatory diet: eliminate processed foods, add turmeric, omega-3s',
            roadCompatible: true
          },
          {
            priority: 2,
            category: 'supplements',
            recommendation: 'Curcumin 500mg, Algae Omega-3 2g, Probiotics 50B CFU',
            roadCompatible: true
          },
          {
            priority: 3,
            category: 'lifestyle',
            recommendation: 'Stress reduction techniques and improved sleep hygiene',
            roadCompatible: true
          }
        ]
      })
    ...
    // Anemia Pattern
    const anemiaMarkers = ['hemoglobin', 'hematocrit', 'rbc', 'iron', 'ferritin']
    const anemiaAssessments = assessments.filter(a => 
      anemiaMarkers.some(marker => 
        a.testName.toLowerCase().includes(marker)
      )
    )

    const lowHemoglobin = anemiaAssessments.find(a => 
      a.testName.toLowerCase().includes('hemoglobin') && a.functionalStatus === 'low'
    )
    const lowIron = anemiaAssessments.find(a => 
      (a.testName.toLowerCase().includes('iron') || a.testName.toLowerCase().includes('ferritin')) && 
      a.functionalStatus === 'low'
    )

    if (lowHemoglobin || (anemiaAssessments.filter(a => a.functionalStatus === 'low').length >= 2)) {
      patterns.push({
        ...
        truckDriverImpact: 'Fatigue, weakness, and reduced cognitive function affecting driving safety',
        interventions: [
          {
            priority: 1,
            category: 'medical',
            recommendation: 'Rule out sources of blood loss, comprehensive iron studies',
            roadCompatible: false
          },
          {
            priority: 2,
          ...
          },
          {
            priority: 3,
            category: 'nutrition',
            recommendation: 'Increase red meat, organ meats, and iron-rich foods',
            roadCompatible: true
          }
        ]
      })
  ...
  private assessDOTRisks(assessments: LabAssessment[]): FunctionalAnalysisResult['dotRisks'] {
    ...
    assessments.forEach(assessment => {
      ...
      } else if (assessment.truckDriverRisk?.priority === 'high' && 
    ...
    if (glucoseAssessment && glucoseAssessment.value > DOT_CRITICAL_VALUES.glucose.threshold) {
      immediate.push(`Glucose ${glucoseAssessment.value} mg/dL exceeds DOT threshold - diabetes evaluation required`)
      careerThreat = true
    }
  ...
  private calculateOverallHealth(assessments: LabAssessment[], patterns: HealthPattern[]): FunctionalAnalysisResult['overallHealth'] {
    const totalAssessments = assessments.length
    const optimalCount = assessments.filter(a => a.status === 'optimal').length
    const functionalCount = assessments.filter(a => a.status === 'functional').length
    ...
    if (score >= 90) {
      ...
      grade = 'D'
      summary = 'Poor health - multiple dysfunction patterns require intervention'
    } else {
      grade = 'F'
      summary = 'Critical health concerns - immediate medical attention recommended'
    }

    return { score: Math.round(score), grade, summary }
  }

  private generateRecommendations(assessments: LabAssessment[], patterns: HealthPattern[]): FunctionalAnalysisResult['recommendations'] {
    const immediate: string[] = []
    const shortTerm: string[] = []
    const longTerm: string[] = []

    // Add critical finding recommendations
    assessments.filter(a => a.status === 'critical').forEach(assessment => {
      immediate.push(`Address critical ${assessment.testName} level - consult physician immediately`)
    })
    ...
    const concerningAssessments = assessments.filter(a => 
      a.status === 'concerning' && a.truckDriverRisk?.priority === 'high'
    )

    if (concerningAssessments.length > 0) {
      shortTerm.push('Schedule comprehensive DOT medical evaluation')
      shortTerm.push('Implement truck cab exercise routine for metabolic health')
      longTerm.push('Consider career sustainability planning with health optimization')
    }

    // Add general truck driver health recommendations
    if (patterns.length > 0) {
      longTerm.push('Develop road-compatible meal prep system for consistent nutrition')
      longTerm.push('Install blackout curtains and white noise for better sleep quality')
      longTerm.push('Create stress management routine compatible with driving schedule')
    }

    return { immediate, shortTerm, longTerm }
  }

  private createNextSteps(assessments: LabAssessment[], patterns: HealthPattern[], criticalFindings: LabAssessment[]): string[] {
    const steps: string[] = []

    if (criticalFindings.length > 0) {
      steps.push('🚨 Schedule immediate physician consultation for critical findings')
    }
    ...
    if (assessments.some(a => a.truckDriverRisk?.priority === 'high')) {
      steps.push('🚛 Review DOT medical requirements and optimize for certification')
    }

    steps.push('🔄 Retest key markers in 6-8 weeks to monitor progress')
    steps.push('📞 Schedule follow-up consultation to review protocol effectiveness')

    return steps
  }

  private async saveAnalysis(documentId: string, result: FunctionalAnalysisResult): Promise<void> {
    try {
      await prisma.medicalDocument.update({
        where: { id: documentId },
        data: {
          metadata: {
            ...(await prisma.medicalDocument.findUnique({ where: { id: documentId }, select: { metadata: true } }))?.metadata,
            ...
            dotRiskLevel: result.dotRisks.careerThreat ? 'high' : result.dotRisks.immediate.length > 0 ? 'medium' : 'low'
          }
        }
      })
