 ...
 */
function extractSupplementsFromAnalysis(
  analysisText: string
): ExtractedSupplement[] {
  ...
  for (const sectionPattern of supplementSections) {
    ...
    if (sectionMatch && sectionMatch[1]) {
      ...
      console.log(
        'Found supplement section:',
        supplementText.substring(0, 200) + '...'
      );
      break;
    }
  }
  ...
  for (const pattern of supplementPatterns) {
    ...
    while ((match = pattern.exec(supplementText)) !== null) {
      ...
      console.log('Found potential supplement match:', {
        name,
        dosage,
        timing,
      });
      ...
      if (existingIndex === -1) {
        ...
        console.log(
          'Supplement already exists, skipping duplicate:',
          cleanName
        );
      }
    }
    ...
    while ((match = brandPattern.exec(analysisText)) !== null) {
      ...
      if (
        ...
        const dosageMatch = context.match(
          /([0-9]+(?:\.[0-9]+)?\s*(?:mg|g|mcg|IU|capsules?|tablets?|drops?|ml))/i
        );
        const timingMatch = context.match(
          /(once|twice|three times|daily|morning|evening|before meals?|with meals?|after meals?|as directed)/i
        );
...
Based on your comprehensive functional medicine assessment, I've created this personalized protocol to address your unique health needs and support your wellness journey.`;
  ...
  const progressMatch = analysisText.match(
    /(?:progress assessment|overall trajectory|critical concerns?)[:\n]([^#]*?)(?=\n#{1,3}|\n\n[A-Z]|$)/i
  );
  if (progressMatch) {
  ...
  if (!clinicalFocus) {
    const healthConcernMatch = analysisText.match(
      /(primary concerns?|main issues?|key findings?|root cause[s]?)[:\n]([^#]*?)(?=\n#{1,3}|\n\n[A-Z]|$)/i
    );
  ...
  if (!clinicalFocus) {
    const challengesMatch = analysisText.match(
  ...
  if (!clinicalFocus) {
    if (/diabetes|diabetic|glucose|HbA1c/i.test(analysisText)) {
      clinicalFocus =
        'Focused on optimizing glucose control, managing insulin resistance, and preventing diabetic complications through targeted nutritional and lifestyle interventions.';
    } else if (/digestive|gut|microbiome/i.test(analysisText)) {
      clinicalFocus =
        'Comprehensive approach to restore digestive function, balance gut microbiome, and address underlying GI dysfunction patterns.';
    } else if (/thyroid/i.test(analysisText)) {
      clinicalFocus =
        'Targeted support for optimal thyroid function through nutritional optimization and metabolic pathway enhancement.';
    } else if (/adrenal|stress|fatigue/i.test(analysisText)) {
      clinicalFocus =
        'Comprehensive adrenal support protocol to address stress response dysfunction and restore energy balance.';
    } else {
      clinicalFocus =
        'Comprehensive functional medicine approach to address identified health patterns and optimize overall wellness.';
    }
  }
  ...
  console.log(
    'Generated clinical focus:',
    clinicalFocus.substring(0, 100) + '...'
  );
  ...
  try {
    ...
    if (!protocolName || protocolName.trim() === '') {
      ...
      if (!protocolName || protocolName.includes('undefined')) {
        const focusMatch = analysis.fullAnalysis.match(
          /(?:primary|main|key|critical)\s*(?:concern[s]?|issue[s]?|focus|goal)[s]?[:\-\s]*([^\n.]{10,80})/i
        );
      ...
      if (
        !protocolName ||
        protocolName.includes('undefined') ||
    ...
    const extractedSupplements = extractSupplementsFromAnalysis(
      analysis.fullAnalysis
    );

    // Extract metadata
    const { greeting, clinicalFocus } = extractProtocolMetadata(
      analysis.fullAnalysis,
      clientName
    );
    ...
    const dietaryMatch = analysis.fullAnalysis.match(
      /diet[ary]*\s*(?:recommendations?|guidelines?)[:\n]([^#]*?)(?=\n#|\n\n[A-Z]|$)/i
    );
    if (dietaryMatch) {
      dietaryGuidelines = {
        recommendations: dietaryMatch[1]
          .trim()
          .split('\n')
    ...
    const lifestyleMatch = analysis.fullAnalysis.match(
      /lifestyle\s*(?:modifications?|recommendations?)[:\n]([^#]*?)(?=\n#|\n\n[A-Z]|$)/i
    );
    if (lifestyleMatch) {
      lifestyleModifications = {
        recommendations: lifestyleMatch[1]
          .trim()
          .split('\n')
          .filter(line => line.trim()),
        extractedFrom: 'analysis',
    ...
    const prioritySupplements = extractedSupplements
      .slice(0, 5)
      .map(supplement => ({
