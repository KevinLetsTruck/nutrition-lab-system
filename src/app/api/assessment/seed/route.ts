  ...
  try {
    ...
    if (existingTemplate) {
      ...
      return NextResponse.json({
        ...
        stats: {
          ...
          seedOilQuestions: questionBank.filter(q => q.category === QuestionCategory.SEED_OIL).length,
    ...
    return NextResponse.json({
      ...
      stats: {
        totalQuestions: questionBank.length,
        modules: assessmentModules.length,
        seedOilQuestions: questionBank.filter(q => q.category === 'SEED_OIL').length,
...
export async function GET() {
  try {
    ...
    const stats = {
      ...
      totalQuestions: Array.isArray(template.questionBank) ? template.questionBank.length : 0,
