...
export async function GET(request: NextRequest) {
  ...
  try {
    // Direct updates with the correct URLs
    const updates = [
      {
    ...
    return NextResponse.json({
      success: true,
      message: 'URLs updated with correct Supabase URL',
      correctUrl: 'https://ajwudhwruxxdshqjeqij.supabase.co',
      results,
      nextStep: 'Go back to your client page and refresh - PDFs should now work!'
    })
  } catch (error) {
    return NextResponse.json({ 
