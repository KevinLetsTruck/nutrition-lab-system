...
export default function SimpleAssessmentTest() {
  ...
  const addLog = (msg: string, type: 'info' | 'success' | 'error' = 'info') => {
    ...
    setLog(prev => [...prev, `${new Date().toLocaleTimeString()}: ${prefix} ${msg}`]);
  };
    ...
    try {
      ...
      try {
        ...
        if (data.success && data.data) {
          const { assessmentId: newId, firstQuestion, status: assessmentStatus } = data.data;
  ...
  const resetAnswer = (question: any) => {
    ...
    switch (question?.type) {
      case 'LIKERT_SCALE':
        setSelectedAnswer('3'); // Middle value for 5-point scale
        break;
      case 'YES_NO':
        setSelectedAnswer('');
        break;
  ...
  const submitAnswer = async () => {
    ...
    // Validate answer
    if (currentQuestion.type === 'MULTI_SELECT' && multiSelectAnswers.length === 0) {
    ...
    addLog(`Submitting answer: ${JSON.stringify(answerValue)} for question ${currentQuestion.id}`);

    try {
      // First, submit the response
      const submitRes = await fetch(`/api/assessment/test/${assessmentId}/response`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      if (!submitRes.ok) {
        ...
        addLog(`Failed to submit: ${errorData.error || 'Unknown error'}`, 'error');
      ...
      if (submitData.success) {
        ...
        const nextRes = await fetch(`/api/assessment/test/${assessmentId}/next-question`, {
          method: 'GET',
          headers: { 'Content-Type': 'application/json' }
        });
        ...
        if (!nextRes.ok) {
          ...
          if (nextData.success && nextData.data?.question) {
            ...
            addLog(`Next question loaded: ${nextData.data.question.id}`, 'success');
            addLog(`Question type: ${nextData.data.question.type}`, 'info');
            setMessage(`Question ${questionsAnswered + 1} answered. Moving to next...`);
  ...
  const getLikertLabels = (question: any) => {
    ...
    if (question.options && Array.isArray(question.options) && question.options.length >= 2) {
      return {
        ...
        max: question.options[1]?.label || question.options[question.options.length - 1]?.label || 'Highest'
      };
    }
  ...
  const renderQuestionInput = () => {
    ...
    switch (questionType) {
      case 'LIKERT_SCALE':
        // Convert to 5-point scale with radio buttons for consistency
        let scaleOptions = [
        ...
        // Use custom labels if provided in the question
        const labels = getLikertLabels(currentQuestion);
        if (labels.min && labels.max) {
          // For questions with specific min/max labels, create a proper scale
          scaleOptions = [
            { value: '1', label: labels.min },
            { value: '2', label: 'Low' },
            { value: '3', label: 'Moderate' },
            { value: '4', label: 'High' },
            { value: '5', label: labels.max }
          ];
        }
        
        // Override for specific question types based on the text
        if (currentQuestion.text?.toLowerCase().includes('how often') || 
            currentQuestion.text?.toLowerCase().includes('frequency')) {
          scaleOptions = [
            { value: '1', label: 'Never' },
        ...
        return (
          <RadioGroup value={selectedAnswer.toString()} onValueChange={(value) => setSelectedAnswer(value)}>
            <div className="space-y-3">
              {scaleOptions.map((option, index) => (
                <div key={index} className="flex items-start space-x-2">
                  <RadioGroupItem value={option.value} id={`scale-${index}`} />
                  <Label htmlFor={`scale-${index}`} className="cursor-pointer">
          ...
          <RadioGroup value={selectedAnswer} onValueChange={setSelectedAnswer}>
            <div className="space-y-3">
              <div className="flex items-center space-x-2">
                ...
                <Label htmlFor="yes" className="cursor-pointer text-base">Yes</Label>
              </div>
              <div className="flex items-center space-x-2">
                <RadioGroupItem value="no" id="no" />
                <Label htmlFor="no" className="cursor-pointer text-base">No</Label>
              </div>
            </div>
          </RadioGroup>
        );
          ...
          <RadioGroup value={selectedAnswer} onValueChange={setSelectedAnswer}>
            <div className="space-y-3">
              {currentQuestion.options?.map((option: QuestionOption, index: number) => (
                <div key={index} className="flex items-start space-x-2">
                  <RadioGroupItem value={option.value.toString()} id={`mc-${index}`} />
      ...
      case 'FREQUENCY':
        const frequencyOptions = currentQuestion.frequencyOptions || currentQuestion.options || [
          ...
          <RadioGroup value={selectedAnswer} onValueChange={setSelectedAnswer}>
            <div className="space-y-3">
              {frequencyOptions.map((option: any, index: number) => (
                <div key={index} className="flex items-start space-x-2">
                  <RadioGroupItem value={(option.value || index).toString()} id={`freq-${index}`} />
            ...
            <div className="space-y-3">
              {durationOptions.map((option: any, index: number) => (
                <div key={index} className="flex items-start space-x-2">
                  ...
                  <Label htmlFor={`dur-${index}`} className="cursor-pointer text-base">
                    {option.label}
                  </Label>
                </div>
              ))}
          ...
          <div className="space-y-3">
            {currentQuestion.options?.map((option: QuestionOption, index: number) => (
              <div key={index} className="flex items-start space-x-2">
                <Checkbox
                  id={`ms-${index}`}
                  checked={multiSelectAnswers.includes(option.value.toString())}
                  onCheckedChange={(checked) => {
                    if (checked) {
                      setMultiSelectAnswers([...multiSelectAnswers, option.value.toString()]);
                    } else {
                      setMultiSelectAnswers(multiSelectAnswers.filter(v => v !== option.value.toString()));
                    }
                  }}
                />
      ...
      case 'TEXT':
        return (
          <Textarea
            ...
            placeholder={currentQuestion.placeholder || 'Enter your response here...'}
  ...
  return (
    <div className="container mx-auto p-8 max-w-4xl">
      <h1 className="text-3xl font-bold mb-6">Assessment System Test</h1>
        ...
        <CardContent className="space-y-4">
          ...
          {assessmentId && (
            <div className="text-sm text-gray-600 dark:text-gray-400">
              Assessment ID: {assessmentId} | Questions Answered: {questionsAnswered}
            </div>
          )}
        </CardContent>
      </Card>
          ...
          <CardHeader>
            <div className="flex justify-between items-start">
              ...
              <span className={cn(
                ...
                currentQuestion.type === 'LIKERT_SCALE' && "bg-blue-100 dark:bg-blue-900",
                currentQuestion.type === 'YES_NO' && "bg-green-100 dark:bg-green-900",
                currentQuestion.type === 'MULTIPLE_CHOICE' && "bg-purple-100 dark:bg-purple-900",
                currentQuestion.type === 'FREQUENCY' && "bg-orange-100 dark:bg-orange-900",
                currentQuestion.type === 'MULTI_SELECT' && "bg-indigo-100 dark:bg-indigo-900",
                currentQuestion.type === 'TEXT' && "bg-teal-100 dark:bg-teal-900",
                currentQuestion.type === 'NUMBER' && "bg-red-100 dark:bg-red-900"
              )}>
      ...
      {message && (
        <div className="mb-6 p-4 bg-blue-50 dark:bg-blue-900/20 text-blue-900 dark:text-blue-100 rounded">
          {message}
        </div>
      )}
