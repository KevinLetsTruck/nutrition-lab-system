...
export function ProtocolDisplay({
      ...
      <Card>
        <CardContent className="p-6 text-center">
          ...
          <p className="text-gray-600">
            No structured protocol found in this analysis
          </p>
          <p className="text-sm text-gray-500">
            Import a Claude Desktop protocol to see structured recommendations
          </p>
        </CardContent>
      </Card>
    );
  ...
  const letstruckProducts =
    protocol.supplements?.filter((s: any) => s.letstruck_sku)?.length || 0;
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-orange-600">
                {protocol.coachingNotes?.length || 0}
              </div>
            ...
            {protocol.supplements && (
              <div>
                <h4 className="font-medium text-gray-900 mb-3 flex items-center gap-2">
                  ...
                  Supplements (
                  {
                    protocol.supplements.filter(
                      (s: any) => s.phase === (activePhase + 1).toString()
                    ).length
                  }
                  )
                </h4>
                        ...
                        <div className="mt-3 space-y-1">
                          {supplement.letstruck_sku && (
                            <div className="text-xs text-green-700">
                              <strong>LetsTruck:</strong>{" "}
                              {supplement.letstruck_sku}
                            </div>
                          )}
                          {supplement.biotiics_alternative && (
                            <div className="text-xs text-blue-700">
                              <strong>Biotiics:</strong>{" "}
                              {supplement.biotiics_alternative}
                            </div>
                          )}
            ...
            {currentPhase.successCriteria &&
              currentPhase.successCriteria.length > 0 && (
                <div>
                  ...
                  <ul className="space-y-2">
                    {currentPhase.successCriteria.map(
